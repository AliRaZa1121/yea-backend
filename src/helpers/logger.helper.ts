...
export class Logger {
  ...
  private static loggerMiddleware: Function | null = null;

  // Error Handling Logs
  private static log(logLevel: LogLevel, data: any, tag?: string) {
    if (Number(process.env.APP_LOG_LEVEL) > logLevel) {
      return;
    }
  ...
  public static GetLoggerMiddleware() {
    ...
    if (Logger.loggerMiddleware === null && process.env.APP_DEBUG === 'true') {
      Morgan.token('authorization', (req: any): string => {
      ...
      Morgan.token('query', (req: any): string => {
        return req.query ? JSON.stringify(req.query, null, 4) : '';
      });

      Morgan.token('params', (req: any): string => {
